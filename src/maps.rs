    ...
    for trx in block.transaction_traces.iter() {
        ...
        for call in trx.calls.iter() {
            ...
            for log in call.logs.iter() {
                ...
                let type0_balance_changes = find_erc20_balance_changes_type_type1(trx, call, &transfer);
                if !type0_balance_changes.is_empty() {
                    balance_changes.extend(type0_balance_changes);
                    continue;
                } else {
                    let type1_balance_changes = find_erc20_balance_changes_type2(&transfer, trx);
...
/// normal case
fn find_erc20_balance_changes_type_type1(trx: &TransactionTrace, call: &Call, transfer: &Transfer) -> Vec<BalanceChange> {
    let mut out = Vec::new();
...
fn find_erc20_balance_changes_type2(transfer: &Transfer, trx: &TransactionTrace) -> Vec<BalanceChange> {
    let mut out = Vec::new();
